...
# add rows 
data_add <- data.frame(date = seq(from = dataM$date[nrow(dataM)] %m+% months(1), 
                                  by = "months", 
                                  length.out = Nh
                                  ),
                       spr = rep(NA, Nh),
...
data_tmp <- dataM %>% filter(variable %in% c("nber_rec", "spr", "tp_ny")) %>%
                      spread(variable, value) %>%
                      rbind(data_add) %>% 
